(* *)
(* Code PDESpecialSolutionsV4-Traditional-Format-Mar15-2020.m *)
(* Last updated: March 15, 2020 at 14:20 by Hereman in Boulder *)
(* Reasons: Included Robert Kragler's code to print equations in correct format. *)
(* Also added a right hand side (== 0) in the output of the translation of the nonlinear PDE *) 
(* into the nonlinear ODE. *)
(* Changes: see WH:03/15/2020 *)
...
(* Purpose: replace the old AnalyzeAndSolve -- dated January 8, 2010 *)
(* with Doug Baldwin new AnalyzeAndSolve -- dated March 13, 2010     *)
(* Fix various bugs. Corrections are marked with WH:03/17/2010       *)

(* Previously updated: January 8, 2010 by Douglas Baldwin and Willy Hereman *)
(* in Boulder at 16:50 *)
...
(* ************************************************************************ *)
(* : Title : PDESpecialSolutionsV4-Traditional-Format-Mar15-2020.m 
...
*  : Last updated : Saturday, March 15, 2020 at 14:20 by Hereman 
*  : Previously updated : Friday, January 8, 2009 at 16:50 by Hereman 
...
(* Options definition. *)
(* WH:01/23/2009 NumericTest is set to True by default, *) 
(* SymbolicTest is also set to True by default *)
...
PDESpecialSolutions[eqns_List, funcs_List, vars_List, param_List, 
  ...
  Block[
    {testarg = FreeQ[MapAll[Expand, eqns], (Power[b__, a__] /; 
     ...
     time = TimeUsed[],
     system,
     (* WH: 03/15/2020 added local variable systemForm *)
     systemForm, 
     mSoln,
     newSystem,
     solution
  ...
  (
    ...
    (* WH:03/15/2020 used pdeFormKragler *)
    If[VerboseTest,
      Print["The given system of differential equations is: "];
      Print[pdeFormKragler[#& /@ eqns, funcs]]
      ]; 
 (* start old code 
    If[VerboseTest,
      Print["The given system of differential equations is: "];
 ...
 (* WH:03/15/2020 system did not have proper right hand side (== 0) *)
...
(* *********************************************************************** *)
(* WH:03/15/2020 added pdeFormKragler *)
(* ********** start new pdeForm suggested by Robert Kragler ******** *)
pdeFormKragler[expr_, functions_] :=
   Module[{notationrules, FF, xx, nn}, 
   (* Print["Inside pdeFormKragler, functions = ", functions]; *)
   notationrules = 
   {Derivative[nn__][FF_][xx__] :> Subscript[FF, SequenceForm @@ 
                Flatten[ Table[#[[1]], {#[[2]]} ]& /@ Transpose[{{xx}, {nn}}] ] ],
            Sequence @@ ((# -> Head[#])& /@ functions), functions -> Head[functions]
   };
   (* Print["Inside pdeFormKragler, notationrules = ", notationrules]; *)
   (* TableForm[expr /. notationrules] *)
   expr /. notationrules
   ];

(* ********** end new pdeForm suggested by Robert Kragler ******** *)
(* ************************************************************************** *)
      ...
      { Sqrt[xxx__^2] :> 
        ...
        Sqrt[-ttt_*zzz__^2] :> 
          (warn = Append[warn, "Sqrt[-a*b^2]->I*b*Sqrt[a]"];
           I*zzz*Sqrt[ttt]), 
(* WH:09/24/2019 added this rule to test a sech or Sech solution *)
(* did not work !!! *)
(*
        Sech[xx__] :> 
...
*)
      ...
      symbolicTestSolutions = 
        ...
        If[!FreeQ[CleanUpFunction[symbolicTestSolutions], Sech], 
           symbolicTestSolutions = Factor[symbolicTestSolutions /. {Sech[xx__]->Sqrt[1-Tanh[xx]^2]}];
          (* Print["At pt XXXXXXX2, symbolicTestSolutions = ", CleanUpFunction[symbolicTestSolutions]]; *)
           symbolicTestSolutions = Expand[symbolicTestSolutions /. {Tanh[xx__]->Sqrt[1-Sech[xx]^2]}]
          (* ; *)
...
AnalyzeAndSolve::soln = "The solution `1` 
is being removed because it is inconsistent."
...
(* Terminates that recursive branch if the solution explodes or *)
(* indeterminate. *)
...
(* WH: March 13, 2010 put sets of parenthesis around (a) and (b) *)
(* ************************************************************************ *)
...
AnalyzeAndSolve::soln = "The solultion `1` 
is being removed because it is inconsistent."
...
(* Print[" "]; *)
Print["Package PDESpecialSolutionsV4-Traditional-Format-Mar15-2020.m was successfully loaded."];
Print["Last updated by Willy Hereman on March 15, 2020 at 14:20."];
(* Print["Package PDESpecialSolutionsV3.m was successfully loaded."]; *)
(* Print["Package PDESpecialSolutionsV4.m was successfully loaded."]; *)
Print["Based on version 4 (in traditional format), last updated by Hereman on September 24, 2019."]; 
Print["Based on version 3, last updated by Hereman on March 21, 2010."];
(* Print["Version 3, last updated by Hereman on March 21, 2010."]; *)
Print["Previously updated by Goktas on December 13, 2009"];
Print["and by Hereman on January 8 and January 23, 2009."];
Print["Based on code originally written by Baldwin et al., February 28, 2005."];
